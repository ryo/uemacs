...
static char brdcstbuf[ 1024];		/* Broadcast messages */
static int brdcstpos = 0;
static int newbrdcst = 0;
...
1		that will wake us up when the next character is typed */
...
static next_read( flag)
int flag;		/* TRUE to use timeout of 0 */
{
    if( waiting || stalled)
...
static mbreadast()
{
	...
	{	/* Got braodcast, get it */
	    if( brdcstpos+mbmsg.brdcnt > sizeof( brdcstbuf)-1)
	    {	/* Too many messages, trash the last ones */
		mbmsg.brdcnt = sizeof( brdcstbuf)-brdcstpos-1;
	    }
	    memcpy( &brdcstbuf[ brdcstpos], mbmsg.message, mbmsg.brdcnt);
	    brdcstpos += mbmsg.brdcnt;
	    brdcstbuf[ brdcstpos] = 0;
	    RemoveEscapes(brdcstbuf);
	    brdcstpos=strlen(brdcstbuf);
	    newbrdcst = 1;
	}
	else
	{
...
#if 0
    /* I can't agree with the "Don't trash the screen" comment.
     * If you don't want messages to the screen, turn them off on
     * the command line.  Personally, i want to know when i
     * receive mail, and don't want a text editor concealing it
     * from me. (jmg)
     */
...
/*
	...
	be needed on network links.  I'm still experimeinting to get hte
	best numbers.
*/
...
*/
    SYS$SETAST( 0);
    ret = tybuf[ tyout++];
    if( tyout >= tymax) { tyout = 0; tymax = sizeof( tybuf);}
    tylen--;		/* Should be ADD_INTERLOCKED */
    if( stalled) if( tylen < (2*MINREAD))
    {
...
/*
	...
	called up from MAILor NOTES.  Note that it may be called more than
	onc, and that "main()" is never called.
*/

ME$EDIT( infile, outfile)
struct  dsc$descriptor *infile;
struct  dsc$descriptor *outfile;
/*
