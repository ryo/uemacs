...
/*	if (sp == NULL)  */  /* This doesn't work anymore  PGA */
        if (sp == NULL)      /* But, that's what you get with ^G */
           return(NULL);
        if (*sp == '\0')     /* This would be an access violation  */
		return(NULL); /* on some systems */
...

{
	...
	register int index;	/* index into strings to compare */
#if	1	/* new completion code from Michael J. Arena */
