 ...
 */
 /*** Modified so filewrite will use remembered name, if none given.
   - This helps a lot on systems that use xon/xoff  8/10/90  PGA  */
...
/*                     "Write file" */
              {
              if (lastkey == ectoc(abortc))
                return(FALSE);
              if (curbp->b_fname[0] == 0)
		return(FALSE);
               }
        if (fname == NULL) fname = curbp->b_fname;
...

{
	...
	if (restflag)		/* don't allow this command if restricted */
		return(resterr());
	if ((fname = gtfilename(TEXT218)) == NULL)  /* ~/ and $VAR expansion */
/*                     "Append file" */
		return(FALSE);
