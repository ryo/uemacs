...
#define	TEXT3	"[L'exe'cution a e'choue']"			/*"[Execution failed]"*/
...
#define	TEXT8	"[Annule']"					/*"[Aborted]"*/
#define	TEXT9	"[Marque %d pose'e]"				/*"[Mark set]"*/
#define	TEXT10	"[Marque %d enleve'e]"				/*"[Mark removed]"*/
#define	TEXT11	"Pas de marque %d dans cette fene^tre"		/*"No mark %d in this window"*/
#define	TEXT12	"[Fichier Help absent]"				/*"[Help file is not online]"*/
#define	TEXT13	": de'crire-la-touche "				/*": describe-key "*/
#define	TEXT14	"Non lie'e"					/*"Not Bound"*/
...
#define	TEXT18	": touche-a-de'lier "				/*": unbind-key "*/
#define	TEXT19	"[Touche non lie'e]"				/*"[Key not bound]"*/
...
#define	TEXT26	"Tampon a` de'truire"				/*"Kill buffer"*/
#define	TEXT27	"Tampon a` de'truire: "				/*"Kill buffer: "*/
...
#define	TEXT37	"ve'rification de l'existence de %s\n"		/*"checking for existence of %s\n"*/
#define	TEXT38	"cre'ation du re'pertoire %s\n"			/*"making directory %s\n"*/
#define	TEXT39	"en cours de cre'ation de %s\n"			/*"creating %s\n"*/
#define	TEXT40	"cre'ation d'un fichier verrouille' impossible"	/*"could not create lock file"*/
...
#define	TEXT48	"succe`s du kill; le processus existe"		/*"kill was good; process exists"*/
...
#define	TEXT54	"[Macro termine'e anormalement]"			/*"[Macro aborted]"*/
...
#define	TEXT61	"%%de'truire: argument ne'gatif illegal"		/*"%%Negative argument to kill is illegal"*/
...
#define	TEXT70	"[Re'gion copie'e]"				/*"[region copied]"*/
#define	TEXT71	"%%Ce tampon est deja diminue'"			/*"%%This buffer is already narrowed"*/

/*		"%%Must narrow at least 1 full line"*/
#define	TEXT72	"%%La diminution doit e^tre d'au moins une ligne comple`te"
...
#define	TEXT75	"[Tampon e'largi]"				/*"[Buffer is widened]"*/
#define	TEXT76	"Pas de marque dans cette fene^tre"		/*"No mark set in this window"*/
#define	TEXT77	"Bug: marque perdue"				/*"Bug: lost mark"*/
#define	TEXT78	"Recherche avant"				/*"Search"*/
#define	TEXT79	"Pas trouve'"					/*"Not found"*/
#define	TEXT80	"Chaine non initialise'e"			/*"No pattern set"*/
#define	TEXT81	"Recherche arrie`re"				/*"Reverse search"*/
...
#define	TEXT104	"Des tampons ont e'te' modifie's. On quitte malgre' tout"	/*"Modified buffers exist. Leave anyway"*/
...
#define	TEXT111	"Pas de macro indique'e"				/*"No macro specified"*/
#define	TEXT112	"Nume'ro de macro hors limite"			/*"Macro number out of range"*/
#define	TEXT113	"Creation de macro impossible"			/*"Can not create macro"*/
#define	TEXT114	"Nom de la proce'dure: "				/*"Procedure name: "*/
#define	TEXT115 "Executer la proce'dure"			/*"Execute procedure"*/
#define	TEXT116	"Proce'dure inexistante"				/*"No such procedure"*/
#define	TEXT117	"Executer le tampon"				/*"Execute buffer"*/
#define	TEXT118	"Tampon inexistant"				/*"No such buffer"*/
#define	TEXT119	"%%Plus de me'moire pendant la recherche"	/*"%%Out of memory during while scan"*/
...
#define	TEXT123	"%%Plus de me'moire pendant l'exe'cution d'une macro"	/*"%%Out of Memory during macro execution"*/
#define	TEXT124	"%%Directive inconnue"					/*"%%Unknown Directive"*/
#define	TEXT125	"Plus de me'moire pour l'enregistrement d'une macro"	/*"Out of memory while storing macro"*/
...
#define	TEXT129	"Exe'cuter le fichier: "			/*"File to execute: "*/
#define	TEXT130	"Macro non de'finie"				/*"Macro not defined"*/
#define	TEXT131	"Lire le fichier"				/*"Read file: "*/
#define	TEXT132	"Inse'rer le fichier"				/*"Insert file: "*/
...
#define	TEXT137	"Cre'ation du tampon impossible"		/*"Cannot create buffer"*/
...
#define	TEXT146	"Fichier tronque'..a` e'crire ailleurs"		/*"Truncated file..write it out"*/
...
#define	TEXT155	"Ouverture du fichier en e'criture impossible"	/*"Cannot open file for writing"*/
#define	TEXT156	"Erreur a la fermeture du fichier"		/*"Error closing file"*/
#define	TEXT157	"Erreur en e'criture E/S"			/*"Write I/O error"*/
#define	TEXT158	"Erreur en lecture du fichier"			/*"File read error"*/
#define	TEXT159	"%J'ai besoin d'un nume'ro de touche de fonction"	/*"%Need function key number"*/
#define	TEXT160	"%Nume'ro de touche de fonction hors limite"		/*"%Function key number out of range"*/
#define	TEXT161	"Contenu de l'e'tiquette: "			/*"Label contents: "*/
#define	TEXT162	" [o/n]? "					/*" [y/n]? "*/
#define	TEXT163	"pas de valeur par de'faut"			/*"no default"*/
...
#define	TEXT172	"%%Plus de me'moire en mode remplacement"		/*"%%Out of memory while overwriting"*/
#define	TEXT173	"ERREUR VERROUILLAGE: Table de verrouillage pleine"	/*"LOCK ERROR: Lock table full"*/
#define	TEXT174	"Verrouillage impossible, plus de me'moire"		/*"Cannot lock, out of memory"*/
...
#define	TEXT180	"%%Re'solution inexistante"				/*"%%No such resolution"*/
#define	TEXT181	"%%Re'solution interdite pour ce type d'e'cran"		/*"%%Resolution illegal for this monitor"*/
#define	TEXT182	"Variable TERM non de'finie"				/*"Environment variable TERM not defined!"*/
#define	TEXT183	"%s: type de terminal inconnu"				/*"Unknown terminal type %s!"*/
#define	TEXT184	"Description termcap incomple`te (lignes)"		/*"termcap entry incomplete (lines)"*/
#define	TEXT185	"Description termcap incomple`te (colonnes)"		/*"Termcap entry incomplete (columns)"*/
#define	TEXT186	"Description termcap incomple`te"			/*"Incomplete termcap entry\n"*/
#define	TEXT187	"Description du terminal trop importante"		/*"Terminal description too big!\n"*/
#define	TEXT188	"[Termine']"					/*"[End]"*/
#define	TEXT189	"Description du terminal non trouve'e\n"		/*"Cannot find entry for terminal type.\n"*/

/*		"Check terminal type with \"SHOW TERMINAL\" or\n"*/
#define	TEXT190	"Ve'rifiez le type de terminal avec \"SHOW TERMINAL\" ou\n"
...
#define	TEXT196	"Obtention des caracte'ristiques du terminal impossible.\n"

/*		"Cannot modify terminal settings.\n"*/
#define	TEXT197	"Modification des caracte'ristiques du terminal impossible.\n"
...
#define	TEXT203	"Nume'ro de fene^tre hors limite"		/*"Window number out of range"*/
#define	TEXT204	"Destruction de cette fene^tre impossible"	/*"Can not delete this window"*/
#define	TEXT205	"De'coupage impossible d'une fene^tre de %d lignes"	/*"Cannot split a %d line window"*/
#define	TEXT206	"Une seule fene^tre"				/*"Only one window"*/
#define	TEXT207	"Changement impossible"				/*"Impossible change"*/
#define	TEXT208	"[Fene^tre inexistante]"			/*"[No such window exists]"*/
#define	TEXT209	"%%Taille d'e'cran hors limite"			/*"%%Screen size out of range"*/
#define	TEXT210	"%%Largeur d'e'cran hors limite"		/*"%%Screen width out of range"*/
...
#define	TEXT213	"[Cre'ation de la liste des fonctions]"		/*"[Building function list]"*/
#define	TEXT214 "%%Fichier %s inexistant"			/*"%%No such file as %s"*/
#define	TEXT215	": macro-pour-cle' "				/*": macro-to-key "*/
#define TEXT216 "Cannot read/write directories!!!"
#define TEXT217 "[Not available yet under AOS/VS]"
#define TEXT218 "Fichier a` ajouter en fin"			/* "Append file"  */
#define TEXT219 "%%Echec de la macro"				/* "%%Macro Failed" */
