/*
 * $Id: line.c,v 1.22 2017/01/02 18:22:09 ryo Exp $
 *
...
lalloc(int used)
{
	...
	lp->l_size = used;
	lp->l_used = used;
	memset(&lp->l_context, 0, sizeof(lp->l_context));
...
lchange(int flag)
{
	...
	if ((curbp->b_flag & BFCHG) == 0) {	/* First change, so	 */
		int savethis = thisflag;
		int savelast = lastflag;
		exechook(modifyhook);
		thisflag = savethis;
		lastflag = savelast;
	...
	if ((curbp->b_flag & BFCHG) == 0) {
		if (stat(curbp->b_fname, &statbuf)==0) {
			if (curbp->b_mtime != statbuf.st_mtime) {
				if (mlyesno(TEXT219) == FALSE) {
					/* "File has changed since visited or saved. Really edit ?" */
					mlwrite(TEXT8);
					/* "[Aborted]" */
					return FALSE;
				}
 ...
 */
int
lowrite(char c)	/* character to overwrite on current position */
{
...
ldelete(long n, int kflag)
{
	...
	while (n) {
...

		{
			...
			if (nthctype(dotp->l_text,doto+chunk) == CT_KJ2) {
				chunk++;
				n++;
				cp2 = cp1 + chunk;
			}
 ...
 */
int
putctext(char *iline)	/* contents of new line */
{
	int status;
...
/*
 * getclabel: get label of current line
 */
char *
getclabel(void)
{
	...
	lp = curwp->w_dotp;
	return lp->l_context.ctx_label;
}

/*
 * putclabel: replace label of the current line
 */
int
putclabel(char *label)
{
	LINE *lp;

	lp = curwp->w_dotp;
	strncpy(lp->l_context.ctx_label, label, sizeof(lp->l_context.ctx_label));
...
int
kinsert(char c)	/* character to insert in the kill buffer */
{
