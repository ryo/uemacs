...

{
        ...
        register int status;		/* status return */
        char bufn[NBUFN+2];		/* name of buffer to execute */
	char *sp;			/* pointer to proc name	*/
	
	/* find out what buffer the user wants to execute */
	/* use buffer completion routine. First char of buffer name must be	*/
	/* a '[' for completion to work. - 02/90, bd -				*/
	if((sp=complete(TEXT115,NULL,CMP_BUFFER,NBUFN))==NULL)
/*                            "Execute procedure" */
		return(FALSE);

	if(*sp == '['){
		/* we have a good buffer name */
		strcpy(bufn,sp);
	} else {
		/* construct the buffer name */
		bufn[0] = '[';
...

{
	...
	/* find out what buffer the user wants to execute 	*/
	/* using buffer completion routine -02/90 , bd-		*/
	if((bp=getcbuf(TEXT117,NULL,FALSE))==NULL){
/*                            "Execute buffer" */
		mlwrite(TEXT118);
/*                      "No such buffer" */
                return(FALSE);
...
dinput:	outline[term.t_ncol - 1] = 0;
	...
	key = getbind(c);
	if (key && key->k_type == BINDFNC && key->k_ptr.fp == meta) /* jpr */
		macbug = FALSE;
