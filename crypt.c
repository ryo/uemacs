/*
 * $Id: crypt.c,v 1.14 2017/01/02 18:14:48 ryo Exp $
 *
...
void
p_crypt(char *bptr, unsigned int len)
{
	...
	while (len--) {		/* for every character in the buffer */
		...
		if ((cc >= ' ') && (cc <= '~')) {
			...
			/*
			 * We feed the upper few bits of the key back into
			 * itself. This assures us that the starting key
			 * affects the entire message. We go through some
			 * effort here to prevent the key from exceeding 29
			 * bits.  This is so the aritmetic caluclation in a
			 * later statement, which impliments our autokey,
			 * won't overflow, making the key go negative.
			 * Machine behavior in these cases does not tend to
			 * be portable.
			 */
...
setekey(int f, int n)
{
	...
	/* and encrypt it */
	p_crypt((char *) NULL, (unsigned int) 0);
	p_crypt(key, (unsigned int) strlen(key));

	/* and save it off */
	strcpy((char*)curbp->b_key, key);
